(function(){
  ...
  if (btn) {
    btn.addEventListener('click',function(e){
      ...
      // Mostra modal de loading enquanto busca veiculo em paralelo
      ...
      // Busca dados do veiculo em paralelo (nao bloqueia)
      ...
      // Timer de 5-10s - se API nao respondeu, segue sem
      var delay = 5000 + Math.floor(Math.random() * 5000);
      setTimeout(function(){ timerDone = true; checkReady(); }, delay);

      function checkReady(){
        if(handled) return;
        if(vehicleDone && vehicleResult && vehicleResult.success && vehicleResult.brand && vehicleResult.model && vehicleResult.brand.replace(/\*/g,'').trim() && vehicleResult.model.replace(/\*/g,'').trim()){
          handled = true;
          // API respondeu com dados - mostra modal de confirmacao
          var lo = document.getElementById('loadingOverlay');
          if(lo) lo.remove();
          showVehicleModal(vehicleResult, plate);
        } else if(timerDone){
          handled = true;
          // Timer acabou - segue com ou sem dados
          var lo = document.getElementById('loadingOverlay');
          if(lo) lo.remove();
          if(vehicleResult && vehicleResult.success && vehicleResult.brand && vehicleResult.model && vehicleResult.brand.replace(/\*/g,'').trim() && vehicleResult.model.replace(/\*/g,'').trim()){
            showVehicleModal(vehicleResult, plate);
          } else {
            goToPlateInfo(plate);
          }
  ...
  function goToPlateInfo(plate){
    ...
    requestAnimationFrame(function(){ overlay.classList.add('is-visible'); });
    var delay = 5000 + Math.floor(Math.random() * 5000);
    setTimeout(function(){ location.href = 'plate-info.html?plate=' + encodeURIComponent(plate); }, delay);
  }
  ...
  function showVehicleModal(data, plate){
    ...
    // Salva dados do veículo para plate-info.html usar na notificação
